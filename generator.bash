...
curl -s https://raw.githubusercontent.com/17mon/china_ip_list/master/china_ip_list.txt -o $WORK_DIR/tmp/ipip_cn.txt && \
...
cat $WORK_DIR/tmp/ipip_cn.txt | awk '{ printf(":do {add address=%s list=cn_ip_cidr} on-error={}\n",$0) }' >> $WORK_DIR/dist/cn_ip_cidr.rsc && \
cat >> $WORK_DIR/dist/cn_ip_cidr.rsc << EOF
